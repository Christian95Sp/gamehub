...
function renderBoard() {
    ...
    */
    ...
    /*    if (humans.B && humans.W) infoDiv.innerHTML = "turno del" + ((player == 'W') ? "l' azzurro" : " viola");
    else infoDiv.innerHTML = humans[player] ? "fai la tua mossa" : "AI sta pensando...";
    if (legalMoves.length === 0) infoDiv.innerHTML = ((player == 'W') ? "VIOLA" : "AZZURRO") + " VINCE";
} */
const wrapper = document.getElementById("wrapper");
    
    // Rimuoviamo le classi del turno precedente
    wrapper.classList.remove("turn-viola", "turn-azzurro", "victory-glow");

    if (legalMoves.length === 0) {
        // CASO VITTORIA
        infoDiv.innerHTML = ((player == 'W') ? "VIOLA" : "AZZURRO") + " VINCE";
        wrapper.classList.add("victory-glow"); // Attiva l'effetto rotante
    } else {
        // CASO TURNO NORMALE
        if (humans.B && humans.W) {
            infoDiv.innerHTML = "Turno del" + ((player == 'W') ? "l' azzurro" : " viola");
        } else {
            infoDiv.innerHTML = humans[player] ? "Fai la tua mossa" : "AI sta pensando...";
        }

        // Applica il colore in base a chi deve muovere ora
        if (player == 'B') {
            wrapper.classList.add("turn-viola");
        } else {
            wrapper.classList.add("turn-azzurro");
        }
    }
}
